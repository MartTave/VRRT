...
def camera_reader(camera_left_id, camera_right_id, frame_width, frame_height, fps):
    cap_left = get_capture(camera_left_id, frame_width=frame_width, frame_height=frame_height, fps=fps)
    cap_right = get_capture(camera_right_id, frame_width=frame_width, frame_height=frame_height, fps=fps)

    for _ in range(20):
        ret, _ = cap_left.read()
        assert ret
        ret, _ = cap_right.read()
        assert ret
    ...
    while not stop_event.is_set():
        start = time.time()
        _, frame1 = cap_left.read()
        timestamp1 = datetime.now().strftime("%H:%M:%S.%f")
        _, frame2 = cap_right.read()
