...
def points_not_in_any_box(points, boxes):
    # This method has been partially generated by IA (chat.deepseek.com)
...
class ArrivalLine:
    ...
    def treat_depth(self, depth, person_result, frame, annotate=False):
        # This method will, for each person boxes
        # Find a POI (point of intereset)
        # Get his depth from the depth map
        # And then check the status form the arrival line
        ...
        for i, (box, id) in enumerate(zip(person_boxes.xyxy, person_boxes.id, strict=False)):
            ...
            if self.persons_depth[id]["arrived"]:
                ...
                if np.isnan(res.coef_[0]):
                    continue
                # We check if the slope of the person depth during tha last N iterations
                # Is in the right direction and is big enough to be a runner
                if res.coef_[0] < 0 - self.min_slope and self.reversed or res.coef_[0] > self.min_slope and not self.reversed:
                    ...
                    if poi[0] in self.line[:, 0]:
                        # Here we fit a regression to the line depth
                        # While removing the values present in a person box, to avoid contamination of the data
                        ...
                        # we can then predict the depth of the line behind the person in order to calculate the arrival time correctly
