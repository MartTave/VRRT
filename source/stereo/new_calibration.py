...
def get_charuco_board():
    board = aruco.CharucoBoard((13, 8), 21, 15, get_charuco_dict())
...
def calibrate_from_pics(path, save_results=True):
    ...
    for i, (left, right) in enumerate(zip(left_pics, right_pics)):
        ...
        left_corners, left_ids, left_obj, left_img = get_points(left_img)
        right_corners, right_ids, right_obj, right_img = get_points(right_img)
    ...
    if save_results:
        ...
        np.savez_compressed(calib_path, left_mtx=left_mtx, right_mtx=right_mtx, left_dist=left_dist, right_dist=right_dist, R=R, T=T, E=E, F=F)
